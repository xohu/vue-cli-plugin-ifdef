    ...
    api.chainWebpack(config => {
        ...
        cssLang.forEach(lang => {
            ...
            cssTypes.forEach(type => {
                langRule.oneOf(type).use(`uniapp-preprocss`).loader(resolve('./lib')).options(cssPreprocessOptions).before('css-loader') // 在 css-loader 之后条件编译一次，避免 import 进来的 css 没有走条件编译
                

              if (loader) { // 在 scss,less,stylus 之前先条件编译一次
                langRule.oneOf(type).use(`uniapp-preprocss-` + lang).loader(resolve('./lib')).options(cssPreprocessOptions).after(loader)
              }
            })
        });

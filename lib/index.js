...
module.exports = function (config, options) {
    ...
    if (options.context && Object.keys(options.context).length) {
        const { check = true, log = check, css = check, js = check, html = check, file = check } = { ...options }

        js && config.module.rule('js').use('ifdef-loader-jsx').loader(resolve('./run')).options({ type: 'js', ...options }).before('cache-loader')
